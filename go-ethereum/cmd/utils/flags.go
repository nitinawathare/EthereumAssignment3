...
var (
	...
	HashPowerFlag = cli.Float64Flag{
		Name:  "hashpower",
		Usage: "Fraction of Hash power controlled by the node",
		Value: eth.DefaultConfig.HashPower,
	}
	InterArrivalFlag = cli.Float64Flag{
		Name:  "interarrival",
		Usage: "Global expected inter arrival time between blocks",
		Value: eth.DefaultConfig.InterArrival,
	}
	BehaviorFlag = cli.Uint64Flag{
		Name:  "behavior",
		Usage: "Strategy followed by the mine (default = 0:Honest; 1:Skips Validation)",
		Value: eth.DefaultConfig.Behavior,
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	if ctx.GlobalIsSet(BehaviorFlag.Name) {
		cfg.Behavior = ctx.GlobalUint64(BehaviorFlag.Name)
	}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	config, _, hashPower, interArrival, behavior, err := core.SetupGenesisBlock(chainDb, MakeGenesis(ctx))
	if err != nil {
		Fatalf("%v", err)
	}
	...
	chain, err = core.NewBlockChain(chainDb, cache, config, hashPower, interArrival, behavior, engine, vmcfg, nil)
	if err != nil {
