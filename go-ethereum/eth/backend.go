...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	chainConfig, genesisHash, _, _, _, genesisErr := core.SetupGenesisBlockWithOverride(chainDb, config.Genesis, config.ConstantinopleOverride)
	...
	eth.blockchain, err = core.NewBlockChain(chainDb, cacheConfig, chainConfig, config.HashPower, config.InterArrival, config.Behavior, eth.engine, vmConfig, eth.shouldPreserve)
	if err != nil {
		return nil, err
	}
...
func (s *Ethereum) InterArrival() float64              { return s.interArrival }
func (s *Ethereum) HashPower() float64                 { return s.hashPower }
func (s *Ethereum) Behavior() uint64                   { return s.behavior }
